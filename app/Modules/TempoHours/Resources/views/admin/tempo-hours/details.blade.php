...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-4">Worklogs</h3>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
                            ...
                            <th class="px-6 py-3 bg-gray-50 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Invoice Ready</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($project->worklogs as $worklog)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $worklog->started_at->format('d/m/Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $worklog->issue_key }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900">{{ $worklog->description }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900">{{ number_format($worklog->time_spent_seconds / 3600, 2) }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">No worklogs found for this period.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

<div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            <div>
                <select 
                    ...
                    wire:loading.attr="disabled"
                >
                    <option value="">All Types ({{ $typeCounts['total'] }})</option>
                    <option value="employee">Employees ({{ $typeCounts['employee'] }})</option>
                    <option value="external">External Resources ({{ $typeCounts['external'] }})</option>
                </select>
            </div>
            <div>
                <button 
                    wire:click="refreshJiraUsers" 
                    class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    wire:loading.attr="disabled"
                >
                    <svg wire:loading.remove wire:target="refreshJiraUsers" class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                    </svg>
                    <svg wire:loading wire:target="refreshJiraUsers" class="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <span wire:loading.remove wire:target="refreshJiraUsers">Sync Employees</span>
                    <span wire:loading wire:target="refreshJiraUsers">Syncing...</span>
                </button>
            </div>
        </div>
    </div>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        ...
                                        <div class="flex-shrink-0 h-10 w-10">
                                            ...
                                            @else
                                                <div class="h-10 w-10 rounded-full bg-indigo-100 dark:bg-indigo-800 flex items-center justify-center">
                                                    <span class="text-indigo-800 dark:text-indigo-100 font-medium text-sm">{{ substr($user->name, 0, 2) }}</span>
                                                </div>
                                            @endif
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                        @if($user->type === 'employee') bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    @if($user->external_url)
                                        <a href="{{ $user->external_url }}" target="_blank" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-3">
                                            View in Jira
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 text-center">
                                    ...
                                    @else
                                        Click the "Sync Employees" button to fetch employees from Jira.
                                    @endif
                                </td>
                            </tr>
                        @endforelse
    ...
    <!-- Loading indicator -->
    <div wire:loading.delay wire:target="search, sortBy" class="fixed inset-0 bg-black bg-opacity-25 flex items-center justify-center z-50">
